#include "include/aes.h"

#include <stdlib.h>
#include <memory.h>

#define Nb 4
#define Nr 10
#define Nk 4

#pragma region TABLES
const static uint8_t mul2[256] = {
	};

const static uint8_t mul3[256] = {
	};
const static uint8_t mul9[256] = {
	};
const static uint8_t mul11[256] = {
	};
const static uint8_t mul13[256] = {
	};
const static uint8_t mul14[256] = {
	};

const static uint8_t rcon[256] = {
	};

const static uint8_t s_box[256] = {
	};
#pragma endregion TABLES
uint8_t *Encrpyt (char *message, char *key, int messageLength)
{
  int mod = messageLength % 16;
  size_t paddedSize = messageLength + 16 - mod;
  size_t noBlocks = paddedSize / 16;
  uint32_t *state = (uint32_t *)malloc (paddedSize);
  memset (state, 0, paddedSize);
  memcpy (state, message, messageLength);

  uint32_t *keyExpanded = KeyExpansion ((uint8_t *)key);

  for (uint32_t *currentState = state; currentState < state + paddedSize; currentState += 16)
	{
	  InitialRound (currentState, keyExpanded);

	  for (int i = 1; i < 10; i++) //TODO verify
		Round (currentState, keyExpanded + Nb * i * sizeof (uint32_t));

	  FinalRound (currentState, keyExpanded + Nb * Nr * sizeof (uint32_t));
	}
}

uint32_t *KeyExpansion (const uint8_t *originalKey)
{
  size_t size = Nb * (Nr + 1);
  uint32_t *W = (uint32_t *)malloc (sizeof (uint32_t) * size);
  for (int currentBlock = Nk; currentBlock < Nb * (Nr + 1);
	   currentBlock++)
	{
	  uint32_t temp = W[currentBlock - 1];
	  if (currentBlock % Nk ==
		  0)
		{ // if its the first the start of a coloumn
		  SubByte ((uint8_t *)&temp, sizeof (uint32_t));
		  temp = RotByte (temp) ^ rcon[currentBlock / Nk];
		}
	  W[currentBlock] = W[currentBlock - Nk] ^ temp;
	}
  return W;
}

void InitialRound (uint32_t *state, uint32_t *roundKey)
{
  AddRoundKey (state, roundKey);
}
void Round (uint32_t *state, uint32_t *roundKey)
{
  SubByte ((uint8_t *)state, 16);
  ShiftRow (state);
  MixColumn (state);
  AddRoundKey (state, roundKey);
}
void FinalRound (uint32_t *state, uint32_t *roundKey)
{
  SubByte ((uint8_t *)state, Nk * Nb);
  ShiftRow (state);
  AddRoundKey (state, roundKey);
}

void AddRoundKey (uint32_t *state, const uint32_t *roundKey)
{
  for (int i = 0; i < 4; i++)
	state[i] = state[i] ^ roundKey[i];
}

void ShiftRow (uint32_t *state)
{
  // Cyclically shift ofver to the left
  uint8_t *tmp = (uint8_t *)malloc (Nk * sizeof (uint8_t));

  for (uint8_t *block_start = (uint8_t *)state;
	   block_start < (uint8_t *)state + Nb * Nk;
	   block_start++)
	{
	  for (int row = 0; row < 3; row++)
		{
		  int Ci = 0;
		  if (row == 1)
			Ci = 1;
		  else if (row == 2)
			Ci = 2;
		  else
			Ci = 3;

		  memcpy (tmp, state, 4);

		  for (int col = 0; col < 4; col++)
			block_start[Nk * row + (col + Nb - Ci) % Nb] = tmp[col];
		}
	}
  //TODO unroll loop, this code was used when implmenbting a complete rijindael
  free (tmp);

}
void MixColumn (uint32_t *bstate)
{
  uint8_t *state = (uint8_t *)bstate;
  uint8_t tmp[16] = {0};

  tmp[0] = (uint8_t)(mul2[state[0]] ^ mul3[state[1]] ^ state[2] ^ state[3]);
  tmp[1] = (uint8_t)(state[0] ^ mul2[state[1]] ^ mul3[state[2]] ^ state[3]);
  tmp[2] = (uint8_t)(state[0] ^ state[1] ^ mul2[state[2]] ^ mul3[state[3]]);
  tmp[3] = (uint8_t)(mul3[state[0]] ^ state[1] ^ state[2] ^ mul2[state[3]]);

  tmp[4] = (uint8_t)(mul2[state[4]] ^ mul3[state[5]] ^ state[6] ^ state[7]);
  tmp[5] = (uint8_t)(state[4] ^ mul2[state[5]] ^ mul3[state[6]] ^ state[7]);
  tmp[6] = (uint8_t)(state[4] ^ state[5] ^ mul2[state[6]] ^ mul3[state[7]]);
  tmp[7] = (uint8_t)(mul3[state[4]] ^ state[5] ^ state[6] ^ mul2[state[7]]);

  tmp[8] = (uint8_t)(mul2[state[8]] ^ mul3[state[9]] ^ state[10] ^ state[11]);
  tmp[9] = (uint8_t)(state[8] ^ mul2[state[9]] ^ mul3[state[10]] ^ state[11]);
  tmp[10] = (uint8_t)(state[8] ^ state[9] ^ mul2[state[10]] ^ mul3[state[11]]);
  tmp[11] = (uint8_t)(mul3[state[8]] ^ state[9] ^ state[10] ^ mul2[state[11]]);

  tmp[12] = (uint8_t)(mul2[state[12]] ^ mul3[state[13]] ^ state[14] ^ state[15]);
  tmp[13] = (uint8_t)(state[12] ^ mul2[state[13]] ^ mul3[state[14]] ^ state[15]);
  tmp[14] = (uint8_t)(state[12] ^ state[13] ^ mul2[state[14]] ^ mul3[state[15]]);
  tmp[15] = (uint8_t)(mul3[state[12]] ^ state[13] ^ state[14] ^ mul2[state[15]]);

  memcpy (state, tmp, 16);
}

uint32_t RotByte (uint32_t word)
{
  uint8_t arr[4] = {};
  memcpy (arr, (uint8_t *)&word, 4);
  uint8_t byte = arr[0];
  arr[0] = arr[1];
  arr[1] = arr[2];
  arr[2] = arr[3];
  arr[3] = byte;
  return *((uint32_t *)arr);
  //  uint32_t high = word << 8;
  //  uint32_t low = word >> 24;
  //  return high | low; //TODO FIX THIS
}

void SubByte (uint8_t *bytes, size_t count)
{
  for (int i = 0; i < count; i++)
	bytes[i] = s_box[bytes[i]];
}
